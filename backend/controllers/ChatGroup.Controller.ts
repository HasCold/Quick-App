...
class ChatGroupController{
    ...
    static async getAllRecords(req: Request, res: Response){
        try {
            ...
            if(!user || !user.id) return apiRespHandler(res, 401, false, "User not authenticated");
    ...
    static async getUniqueRecord(req: Request, res: Response){
        try {
            ...
            if(req.method != "GET") return apiRespHandler(res, 405, false, "Only GET method is allowed");
            if(!id) return apiRespHandler(res, 404, false, "Id not found")
    ...
    static async store(req: Request, res: Response){
        try {
            ...
            if(!user || !user.id) return apiRespHandler(res, 401, false, "User not authenticated");
            if(!body.title || !body.passcode) return apiRespHandler(res, 404, false, "Something not found");
